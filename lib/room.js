            ...
            this.fbRef.child('residents').on('child_added', function(fbResident) {
                ...
                if (residentId) {
                    this.fbRef.child('residents').child(residentId).on('value', function(fbResident) {
                        var residentIsAway = fbResident.val();
                        console.log('----> ' + residentIsAway);
                        if (residentIsAway === true) {
                            log.info({
                                home: this.homeId,
                                room: this.id
                        ...
                        } else if (residentIsAway === false) {
                            if (this.residentFbRefs[residentId]) {
                            log.info({
                                home: this.homeId,
                                room: this.id
                            }, ' Room: Resident with id ' + residentId + ' is deleted since val() is ' + fbResident.val());
...
Room.prototype.updateRoomTemperature = function() {
    ...
    if (this.netatmo !== null) {
        ...
        netatmoUpToDate = ((Date.now() - (this.netatmo.getTimestamp())) < 30 * 60 * 1000) ? true : false;
