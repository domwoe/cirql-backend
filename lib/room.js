...
function Room(homeId, roomId) {
  ...
  var heating = null;
  log.info({home: this.homeId, room: this.id}, ' room constructed');
  var fbRef = new Firebase(fbBaseUrl+'homes/'+this.homeId+'/rooms/'+this.id);

  /** Create heating controller if room has thermostats */
  var self = this;
  fbRef.child('thermostats').on('value', function(fbThermostats) {
      if(fbThermostats.hasChildren()) {
        if(heating === null) {
          heating = new Heating(self.homeId, self.id);
          log.info({home: self.homeId, room: self.id}, ' heatingMgm initialized');
        }
      }
      else {
        if(heating === null) {
          log.info({home: self.homeId, room: self.id}, ' heatingMgm not initialized');
        }
        else {
          log.info({home: self.homeId, room: self.id}, ' heatingMgm deleted');
           heating.setFbRefOff();
           heating = null;
        }
      }
