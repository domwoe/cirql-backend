...
function Room(homeId, roomId, home) {
    ...
    this.fbRefResidents.child("notifications").on("value", function(residentSnap) {
            ...
            this.fbRef.child('residentStates').child(key).once('value', function(fbResidentState) {
                ...
                if (doNotNotifyBecauseIsAway === false) {
                    this.fbRefResidents.child(key).child("notification").child("devices").once('value', function(devicesSnap) {
                        ...
                        if (devicesSnap.val() !== null) {
                            ...
                            if (devices["ios"] !== null) {
                                var ios = devices["ios"];
                                var token = ios["token"];
                                var lang = 'en'; //refactor if available
                                log.info({
                                    home: this.homeId,
                                    room: this.roomId
                                ...
                                if (token !== null) {
                                    if (this.msg !== null) {
                                        ...
                                        log.info({
                                            home: this.homeId,
                                            room: this.roomId
                                        }, " Roomclimate-Analytics: No airquality messages availabel for the room ");
                                    }
                    ...
                    log.info({
                        home: this.homeId,
                        room: this.roomId
                    }, " Roomclimate-Analytics: Notification is skipped since resident is currently away");
                }
            }, this);
        }
    }
