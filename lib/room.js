...
function Room(homeId, roomId) {
    ...
    this.fbRef.child('usesAutoAway').on('value', function(fbAutoAway) {
        log.info({
            home: this.homeId,
            room: this.id
        }, ' Room: usesAutoAway-is-called');
        if (fbAutoAway.val()) {
            if (fbAutoAway.val() === true) {
                this.usesAutoAway = true;
                this.calcIsAwayAndEta();
            } else if (fbAutoAway.val() === false) {
                this.usesAutoAway = false;
                this.fbRef.child('isAway').set(false);
            } else {
                log.info({
                    home: this.homeId,
                    room: this.id
                }, ' Room: usesAutoAway is unclear');
            }
        }
    }, this);


    this.fbRef.child('residentStates').on('value', function(fbResidentStates) {
        if (fbResidentStates.val()) {
            this.residentStates = fbResidentStates.val();
            if (this.usesAutoAway === true) {
                this.calcIsAwayAndEta();
            }
        }
    }, this);
...
Room.prototype.calcIsAwayAndEta = function() {
    if (this.residentStates !== null) {
        var resStates = this.residentStates;
        ...
        log.info({
                    home: this.homeId,
                    room: this.id
                }, ' Room: residentStatesObj ' + JSON.stringify(resStates));
        for (var residentKey in resStates) {
            var resident = resStates[residentKey];
            log.info({
                    home: this.homeId,
                    room: this.id
                }, ' Room: residentObj ' + JSON.stringify(resident));
...
Room.prototype.updateRoomTemperature = function() {
    ...
    if (this.netatmo !== null) {
        this.fbRef.child('lastExternalSensorUpdate').set(new Date(this.netatmo.getTimestamp()));
