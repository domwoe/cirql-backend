...
function Room(homeId, roomId) {
    ...
    this.fbRefActivity = new Firebase(fbBaseUrl + 'homes/' + this.homeId + '/activity/' + this.id);
    ...
    this.fbRefActivity.on('child_added', function(fbRawLog) {
        if(fbRawLog.name()) {
            ...
            if(id === 'raw') {
                this.activity = new Activity(this.homeId, this.roomId);
            }
        }
    },this);

    this.fbRefActivity.on('child_removed', function(fbRawLog) {
        if(fbRawLog.name()) {
            var id = fbRawLog.name();
            if(id === 'raw') {
                log.info({
                    ...
                    this.fbRef.child('residents').child(residentId).on('value', function(fbResident) {
                        ...
                        if (residentIsConsideredForRoom === true) {
                            ...
                            if (this.residentFbRefs[residentId]) {
                                ...
                                if (this.residentEtas[residentId]) {
                                    delete this.residentEtas[residentId];
                                }
                                if (this.residentAways[residentId]) {
                                    delete this.residentAways[residentId];
                                }
