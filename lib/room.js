...
function Room(homeId, roomId) {
    ...
    this.fbRef.child('isAutoAway').on('value', function(fbAutoAway) {
        if (fbAutoAway.val()) {
...
Room.prototype.calcMinEta = function() {
    ...
    if (minEta !== 60 * 60 * 24 * 7) {
        ...
        log.warn({
            home: this.homeId,
            room: this.id
        }, ' Room: Min Eta can not be computed since there are no residents ');
    }
};
