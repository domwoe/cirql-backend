...
function Nefit(homeId, uuid, userPw, devicePw) {
    ...
    this.fbRef.child('initiateTest').on('value', function(fbTest) {
        ...
        if (fbTest.val() === true) {
            this.fbRef.child('status').set(' Connection test initiated...');
            this.fbRef.child('initiateTest').set(false);
            var self = this;
            this.getOutdoorTemp(function(err, res) {
                if (err) {

                    self.fbRef.child('status').set('Failed with response ' + JSON.stringify(err));
                } else {

                    self.fbRef.child('status').set('Successful with response ' + res);
                    var time = new Date();
                    var that = self;
                    setTimeout(function() {
                        that.fbRef.child('status').set('Last successful test ' + time);
                    }, 60 * 1000);
                }
            });
        }
...
Nefit.prototype.connect = function(cb) {
    ...
    this.fbRef.child('status').set('Connecting... (retry: ' + this.connRetries + ')');


    log.info({
        home: this.homeId,
    }, "NEFIT: Connecting... (retry: " + this.connRetries + ")");
    ...
    post.call(this, '/sessions', data, function(err, res) {
        ...
        if (!err) {
            ...
            if (res.hasOwnProperty('sessionId')) {
                ...
                console.log('SUCCESS');
                this.fbRef.child('status').set('Connection established');
                this.connRetries = 0;
                cb(success);
            }
            ...
            // response has no sessionId
                ...
                this.fbRef.child('status').set('Connection failed with code ' + res.res.statusCode);
                if (this.connRetries < 10) {
                    var self = this;
                    setTimeout(function() {
                        self.connect(cb);
                        self.connRetries++;
                    }, (self.connRetries + 1) * self.connRetries * 60 * 1000);
                }
            }
        ...
        // err
            ...
            this.fbRef.child('status').set('Connection failed with error ' + err);
            if (this.connRetries < 10) {
                var self = this;
                setTimeout(function() {
                    self.connect(cb);
...
Nefit.prototype.getOutdoorTemp = function(cb) {
    ...
    get.call(this, '/system/sensors/temperatures/outdoor_t1', function(err, res) {
        cb(err, res);
    });
};
...
Nefit.prototype.setTemperature = function(radiatorId, temperature, cb) {


    log.info({
        home: this.homeId,
        thermostat: radiatorId,
        class: 'Nefit',
    ...
    var resource = '/heatingCircuits/hc1/radiator/' + radiatorId + '/radiatorAmbTemp';

    var data = {
        value: floatTemperature
    };
    ...
    if (this.wait === true) { // Push request on queue
        this.pushToQueue(radiatorId, 'setTemperature', fn);
...
Nefit.prototype.setTrvAddress = function(trvAddress, radiatorId, cb) {
    ...
    if (this.wait === true) { // Push request on queue
        ...
        this.pushToQueue(radiatorId, 'setTrvAddress', fn);
    } else { // Exectute request now
        this.wait = true;
        log.info({
            home: this.homeId
...
Nefit.prototype.getTrvAddress = function(radiatorId, cb) {
    ...
    if (this.wait === true) { // Push request on queue
        ...
        this.pushToQueue(radiatorId,'getTrvAddress', fn);
    } else { // Exectute request now
        this.wait = true;
...
Nefit.prototype.getTarget = function(radiatorId, cb) {
    ...
    if (this.wait === true) { // Push request on queue
        this.pushToQueue(radiatorId, 'getTarget', fn);
    } else { // Exectute request now
        this.wait = true;
        fn.call(this);
    }
...
Nefit.prototype.setTarget = function(radiatorId, target, cb) {
    ...
    if (this.wait === true) { // Push request on queue
        this.pushToQueue(radiatorId, 'setTarget', fn);
    } else { // Exectute request now
        this.wait = true;
        fn.call(this);
    }
...
Nefit.prototype.handleQueue = function() {
    ...
    if (this.requestQueue.length > 0) {
        this.requestQueue[0].fn.call(this);
...
Nefit.prototype.pushToQueue = function(radiatorId, fnName, fn) {

    // look for previous functions of the same type in the queue and remove them
    // 
    for (var i = 0, j = this.requestQueue.length; i < j; i++) {
        if (this.requestQueue[i].hasOwnProperty('radiatorId') &&
            this.requestQueue[i].hasOwnProperty('fnName') &&
            this.requestQueue[i].hasOwnProperty('fn') ) {
            if (radiatorId === this.requestQueue[i].radiatorId && fnName === this.requestQueue[i].fnName) {
                console.log(fnName +' for '+ radiatorId +' already on queue. Remove it...');
                this.requestQueue.splice(i, 1);
            }
        }
        else {
            console.log('Property missing:');
            console.log('RadiatorId: '+this.requestQueue[i].hasOwnProperty('radiatorId'));
            console.log('fnName: '+this.requestQueue[i].hasOwnProperty('fnName'));
            console.log('fn: '+this.requestQueue[i].hasOwnProperty('fn'));
        }    
    }
