  ...
  this.fbRefNetatmo.child('refreshToken').once('value',function(refreshTokenSnap) {
  	...
  	refreshAccessToken(refreshToken,function(err,res) {
  		if (!err && res && res.hasOwnProperty('access_token')) {
  			...
  			log.warn({home: self.homeId},'NetatmoAPI refreshAccessToken error : '+err);
  		}
	...
	function getList() {
		...
		request(params, function (error, response, body) {
	        if (!error && response.statusCode == 200) {
	           ...
	           log.warn({home: self.homeId},'NetatmoAPI getDevices error : '+error);
	        }
   	 	});
	}
		...
		request(params, function (error, response, body) {
	        if (!error) {
	            ...
	            if (jsonObj.status === 'ok') {
	            	log.info({home: self.homeId},'NetatmoAPI new Data for module : '+moduleId);
	            	...
	            	log.warn({home: self.homeId},'NetatmoAPI getMeasure response status : '+jsonObj.status);
	            }
	        }    
	        else {
	        	log.warn({home: self.homeId},'NetatmoAPI getMeasure error : '+error);
	        }	    
