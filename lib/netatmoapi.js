	        ...
	        if (!error) {
	            ...
	            if (jsonObj.status === 'ok') {
					...
					for (var i = 0; i < type.length; i++) {
						self.fbRefNetatmo.child(stationId).child('modules').child(moduleId).child(type[i]).set(lastMeasurement[i]);
					}
					var date = new Date(parseInt(lastTimestamp)*1000);
					self.fbRefNetatmo.child(stationId).child('modules').child(moduleId).child('timestamp').set(lastTimestamp);
					self.fbRefNetatmo.child(stationId).child('modules').child(moduleId).child('date').set(date+'');
	            }
	            else {
