...
Roomclimate.prototype.notifyAll = function(msg) {
    ...
    for (var key in this.notifyResidents) {
        ...
        if (wantsNotifications === true) {
            this.fbRefResidents.child(key).child("notifications").child("devices").once('value', function(devicesSnap) {
                log.info({
                    home: this.homeId,
                    room: this.roomId
                }, " Roomclimate-Analytics: Check devices now!");
                if (devicesSnap.val() !== null) {
                    var devices = devicesSnap.val();
                    if (devices.ios !== null) {
                        if (devices.ios.token !== null) {
                            ...
                            notifications.send('ios', devices.ios.token, msg + " (" + ventilationTime + " min)");
                            log.info({
                                home: this.homeId,
                                room: this.roomId
                            }, " Roomclimate-Analytics: Notification to IOS device sent with " + devices.ios.token);
