    ...
    this.fbRefAnalytics.child('co2-dec').on('child_added', function(co2DecEvent) {
        ...
        if (co2DecEvent.val() !== null) {
            if (this.co2History.length <= 3) {
                ...
                var listStr = "";
                while (this.co2History.next()) {
                    listStr += ", " + this.co2History.current;
                }
                log.info({
                    home: this.homeId,
                    room: this.roomId
                }, " Roomclimate-Analytics: Add co2-dec event with key " + co2DecEvent.key() + " to linkedlist: " + listStr);
            } else {
                ...
                if (keyOfOldestHistoryEvent !== null && keyOfOldestHistoryEvent !== undefined) {
                    ...
                    var listStr2 = "";
                    while (this.co2History.next()) {
                        listStr2 += ", " + this.co2History.current;
                    }
                    log.info({
                        home: this.homeId,
                        room: this.roomId
                    }, " Roomclimate-Analytics: Delete the oldest co2-dec event with key: " + keyOfOldestHistoryEvent + " from list " + listStr2);
                } else {
                    log.info({
                        home: this.homeId,
                        room: this.roomId
