...
function Roomclimate(homeId, roomId) {
    ...
    this.fbRefAnalytics.child('co2-dec').on('child_added', function(co2DecEvent) {
        ...
        if (co2DecEvent.val() !== null) {
            if (Object.keys(this.co2History).length < 2) {
                ...
                for (var key in this.co2History.keys()) {
                    var co2Event = this.co2History[key];
                    if(co2Event.co2StartTime < oldesttimestamp) {
                        oldestKey = key;
                        oldesttimestamp = co2Event.co2StartTime;
                    }
                }
