...
function Home(id) {
  ...
  */
  ...
  this.fbRef.child('residents').on('child_added', function(fbUser) {
    ...
    log.info({home: this.id}, ' Home: Add a new user with id ' + userId);
   ...
   this.fbRef.child('residents').on('child_removed', function(fbUser) {
    ...
    if (userObj) {
      log.info({home: this.homeId}, ' Home: Delete user with id ' +userId);
      userObj.setFbRefOff();
...
Home.prototype.geocodeAddress = function() {
    ...
    log.info({home: this.id}, 'Geocoding for: '+ completeAddress);

    var self = this;
    gm.geocode(completeAddress, function(err, data) {
        if(err) {
          log.warn({home: self.id}, 'Geocoding Problem with: '+ completeAddress);
        }
        if(data) {
          if(data.status === 'OK') {
            var results = data.results[0];
            log.info({home: self.id}, 'Chosen Address: '+ results.formatted_address);
            var location = results.geometry.location;
            location.formattedAddress = results.formatted_address
            log.info({home: self.id}, 'Chosen Location '+ JSON.stringify(location));
