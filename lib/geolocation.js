    ...
    this.fbRefLoc.child('homeregion').on('value', function(fbAtHome){
        if(fbAtHome.val() != null){
           ...
           userRef.child('isAway').set(!fbAtHome.val());
            ...
            log.info({home: this.homeId, user: this.userId}, ' Geolocation: It is unknown if user is in HomeRegion ');
        }
    }, this);

    this.fbRefLoc.on('value', function(fbData) {
      if(fbData.val()) {
       // if(this.locCoords != null && this.locCoords.lat !== fbData.val().lat && this.locCoords.lng !== fbData.val().lng) {
        this.locCoords = {};
...
Geolocation.prototype.getTravelTime = function(locCoords, homeCoords, mode, callback) {
   ...
   var self = this;
    gm.distance(sLocCoords, sHomeCoords, function(err, data) {
        if(err) {
          log.warn({home: self.homeId, user: self.userId}, ' Geolocation: gm.distance calculation failed with: ' + err);
        }
        if(data) {
