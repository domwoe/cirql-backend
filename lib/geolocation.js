...
Geolocation.prototype.checkHomeAway = function() {
        ...
        if (this.homeRegionSeen === true) {
            if (this.insideHomeRegion !== null && this.homeRegionDate !== null) {
                ...
                } else(this.insideHomeRegion === false) {
                ...
                //Check special cases for home/away correction
                //Case 1: For some reason the region update did not work, so resident is away but system thinks he is still at home!
                // If we get location updates from far away, we can correct that to away!
                if (this.insideHomeRegion === true) {
                    if (this.travelEta > this.minTravelEtaForOverrideHomeRegion) {
                        if (this.travelTimeUpdate > this.homeRegionDate) {
                            this.fbResident.child('isAway').set(true);
                            log.info({
                                home: this.homeId,
                                user: this.userId
                            }, ' Geolocation-Override: We set resident as away by recent traveltime-eta of: ' + this.travelEta);
                        }
                    }
                }
            } else {
            ...
            //Check special cases for home/away correction
            //Case 1: For some reason the region update did not work, so resident is at home but system did not get a region update so far!
            // If we get travelEta from very close, we could overrite it
            if (this.travelEta <= this.maxTravelEtaForOverrideHomeRegion) {
                if (this.travelTimeUpdate > this.regionUpdate) {
                    this.fbResident.child('isAway').set(false);
                    log.info({
                        home: this.homeId,
                        user: this.userId
                    }, ' Geolocation-Override: We set resident as athome by recent traveltime-eta of: ' + this.travelEta);
                }
            }
        };
