            ...
            if (this.lastLocDate > moment().unix() - 5 * 60) {
                if (this.homeCoords !== null) {
                    ...
                    log.warn({
                        home: this.homeId,
                        user: this.userId
                    }, ' Geolocation-Distance: The distance to home is: ' + distance);
                    ...
                    var tempEta = null;
                    if (this.homeCoords && this.lastLocCoords && this.homeCoords.lat && this.homeCoords.lng && this.lastLocCoords.lat && this.lastLocCoords.lng) {
                        var self = this;
                        this.getTravelTime(this.homeCoords, this.lastLocCoords, 'driving', function(err, data) {
                            if (err) {
                                log.warn({
                                    home: self.homeId,
                                    user: self.userId
                                ...
                                var obj2 = {
                                    ...
                                    data: {
                                        homeid: self.homeId,
                                        residentid: self.userId,
                                        distance: distance,
                                        eta: null,
                                        lat: self.lastLocCoords.lat,
                                        lng: self.lastLocCoords.lng
                                    }
                                };
                                storage.save(obj2);
                            } else {
                                ...
                                if (data) {
                                    if (data.status === 'OK') {
                                        tempEta = Math.round(data.duration.value / 60);
                                        var obj1 = {
                                            table: 'geolocation',
                                            data: {
                                                homeid: self.homeId,
        ...
        if (fbRegions.val() !== null) {
            ...
            var obj1 = {
                ...
                data: {
                    ...
                    isfarerthan: isFarerThanTemp,
                    iscloserthan: isCloserThanTemp
                }
            };
            storage.save(obj1);
