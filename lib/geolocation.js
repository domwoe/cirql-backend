...
function Geolocation(homeId, userId) {
    ...
    this.fbRefRegions.on('value', function(fbRegions) {
        if (fbRegions.val() !== null) {
            ...
            // Final Check and ETA calculation
            if (this.isCloserThan <= 500) {
                userRef.child('isAway').set(false);
                this.fbResident.child('eta').set(0);
            } else {
                userRef.child('isAway').set(true);
                this.calculateETA();
            }
    ...
    if (this.travelTimeUpdate > 0 && this.regionUpdate > 0) {
        ...
        if (this.travelTimeUpdate >= this.regionUpdate - 1) {
            ...
            if(this.isCloserThan <= 45000) {
                ...
                if(this.regionUpdate > this.travelTimeUpdate + 15*60) {
...
Geolocation.prototype.calculateETAforRegions = function() {
    var factor = 1000; // 1000 meter/minute  = 60 kmh
    var etaForRegions = 0;
    if (this.isCloserThan !== 90000000) {
        etaForRegions = Math.round(this.isCloserThan / factor);
    } else if (this.isFarerThan !== 0) {
        etaForRegions = Math.round(this.isFarerThan / factor);
    } else {
        log.warn({
            home: this.homeId,
            user: this.userId
