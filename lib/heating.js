...
Heating.prototype.planNextTarget = function() {
	...
	if (self.mode == 'schedule') {
		...
		findNextScheduleItem(schedule, function(e, objNextItem) {
			if (e) {
				log.warn(e);
				return;
			}	
...
/**
 * Finds next schedule item.
 * Callback functions has to be in the form function(error, {index, time})
 * where index is the index of schedule where next item can be found
 * and time is the time in ms from now until that event
 */
function findNextScheduleItem(schedule, cb) {
  if (!_.isEmpty(schedule)) {
	  ...
	  var timeTillItem = 0;
	  var timeTillNextItem = 99999999999;
	  ...
	  for (var ii = 0; ii < schedule.length; ii++) {
	  	...
	  	if (timeTillItem < timeTillNextItem) {
	  		timeTillNextItem = timeTillItem;
	  		index = ii;
	   ...
	   log.info('Time till next item: '+(timeTillNextItem/1000/60).toFixed(0));
	   cb(null, { index: index, time: timeTillNextItem });
  }
