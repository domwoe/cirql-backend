        ...
        this.findScheduleItem(this.schedule, 'next', function(e, objNextItem) {
            ...
            if (self.usesAutoAway) {
                ...
                if (!self.isAway) {
                    ...
                    self.setTimeTillNextTarget((timeTillNextTarget).toFixed(0),' by AutoAway and at home ');
                    ...
                    if (self.eta > 0) {
                        ...
                        self.setTimeTillNextTarget((timeTillNextTarget).toFixed(0),' by AutoAway and not at home ');
                        // Set current Target
            ...
            /** Room is occupied */
                ...
                self.setTimeTillNextTarget((timeTillNextTarget).toFixed(0),' by schedule ');
                log.info({
                    home: self.homeId,
                    room: self.roomId
...
Heating.prototype.setTimeTillNextTarget = function(time, reason) {

    if (time !== null) {
        this.fbRef.child('timeTillNextTarget').set(time);
        log.info({
            home: this.homeId,
            room: this.roomId
        }, ' Heating:  Set timeTillNextTarget ' + time + ' by ' + reason);
    }
