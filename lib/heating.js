...
var fbRef = null;
var fbRefMode = null;
var fbRefIsAutoAway = null;
var fbRefTemperature =  null;
var fbRefEta =  null;
var fbRefVirtualTarget = null;
var fbRefHeatupFactor = null;
var fbRefSchedule = null;
var fbRefAutoSchedule = null;
...
function Heating(homeId, roomId) {
  ...
  fbRef = new Firebase(fbBaseUrl+'homes/'+this.homeId+'/rooms/'+this.roomId);
  fbRefMode = fbRef.child('mode');
  fbRefIsAutoAway = fbRef.child('isAutoAway');
  fbRefTemperature =  fbRef.child('temperature');
  fbRefEta =  fbRef.child('eta');
  fbRefVirtualTarget = fbRef.child('virtualTarget');
  fbRefHeatupFactor = fbRef.child('heatupFactor');
  fbRefSchedule = fbRef.child('schedules').child('schedule');
  fbRefAutoSchedule = fbRef.child('schedules').child('autoSchedule');
  ...
  fbRefMode.on('value', function(fbMode) {
  	log.info({home: this.homeId, room: this.roomId}, ' heating: mode is changed ');
  	var mode = fbMode.val();
  	this.mode = mode;
  	switch (mode) {
  		case 'manual':
  ...
  fbRefIsAutoAway.on('value', function(fbData) {
  ...
  fbRefTemperature.on('value', function(fbData) {
 ...
 fbRefEta.on('value', function(fbData) {
  	this.eta = fbData.val();
  	this.planNextTarget();
  		
  }, this);

  fbRefVirtualTarget.on('value', function(fbData) {
  	var first = true;
  	if (!first) {
  ...
  fbRefHeatupFactor.on('value', function(fbData) {
  	this.heatupFactor = fbData.val();
  	this.planNextTarget();
  }, this);

  fbRefSchedule.on('value', function(fbData) {
  	this.schedule = fbData.val();
  	this.planNextTarget();
  }, this);

  fbRefAutoSchedule.on('value', function(fbData) {
...
Heating.prototype.setFbRefOff = function() {
	...
	fbRefMode.off();
	fbRefIsAutoAway.off();
	fbRefTemperature.off();
	fbRefEta.off();
	fbRefVirtualTarget.off();
	fbRefHeatupFactor.off();
	fbRefSchedule.off();
	fbRefAutoSchedule.off();
	log.info({home: this.homeId, room: this.roomId}, ' Heating: All fbRefs are set to off');
};
