...
Heating.prototype.planNextTarget = function() {
	...
	if (self.mode == 'schedule') {
		...
		findScheduleItem(schedule, 'next',function(e, objNextItem) {
			...
			log.info({home: self.homeId, room: self.roomId}, ' Heating: Time till next schedule event: '+(timeTillNextTarget/1000/60).toFixed(0));
...
function findScheduleItem(schedule, which, cb) {
  if (!_.isEmpty(schedule)) {
	  ...
	  for (var scheduleEventKey in schedule) {
	   ...
	   cb(null, { key: key, time: timeTillWantedItem });
  }
