...
function Heating(homeId, roomId) {
    ...
    this.fbRefActivity = new Firebase(fbBaseUrl + 'homes/' + this.homeId + '/activity/rooms/' + this.roomId);
            ...
            if (self.usesAutoAway) {
                ...
                if (self.isAway === false) {
                    ...
                    if (self.eta > 0) {
                        ...
                        // Set current Target
                        if(aaTarget < self.currentScheduledTarget) {                            
                            var msg = 'Reduced setpoint (' + aaTarget + '°) for saving energy';
                            var eventData = helper.createEvent('autoaway-leaving', msg);
                            self.fbRefActivity.push(eventData);
                        }
                        else if (aaTarget > self.currentScheduledTarget) {
                            var msg = 'Reduced setpoint (' + aaTarget + '°) for saving energy';
                            var eventData = helper.createEvent('autoaway-coming', msg);
                            self.fbRefActivity.push(eventData);
                        }
...
Heating.prototype.reactToManualChange = function(target) {
    ...
    var msg = 'Someone set the setpoint to ' + target + ' on a thermostat';
    var eventData = helper.createEvent('manualChange', msg);
