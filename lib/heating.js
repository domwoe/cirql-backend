...
function Heating(homeId, roomId) {
    ...
    this.fbRef.once('value', function(fbData) {
        ...
        this.fbManualTarget.on('value', function(data) {
            if (data.val() !== null) {
            ...
            switch (mode) {
                case 'manual':
                    ...
                    if (this.manualTarget !== null) {
                        this.setTarget(this.manualTarget, ' manual mode ');
                        this.setVirtualTarget(this.manualTarget, ' manual mode ');
                    } else {
                        this.setTarget(this.virtualTarget, ' manual mode init');
                        this.fbManualTarget.set(this.virtualTarget);
                    }
                    break;
                case 'auto':
                    var self = this;
        ...
        this.fbRefScheduleOverride.on('value', function(fbData) {
            if (fbData.val !== null) {
                this.isScheduleOverride = true;
                this.scheduleOverrideTarget = fbData.val();
            } else {
                this.isScheduleOverride = false;
                this.scheduleOverrideTarget = null;
            }
        ...
        this.fbRefVirtualTarget.on('value', function(fbData) {
            if (this.mode !== null) {
                if (this.mode === 'manu') {
                    ...
                    this.fbManualTarget.set(fbData.val());
    ...
    if (self.mode === 'manu') {
        if (self.usesAutoAway !== null && self.usesAutoAway === true) {
            if (self.isAway === false) {
                self.setTarget(self.virtualTarget, 'autoaway-manual (at home)');

            } else if (self.isAway === true) {
                aaTarget = self.calcAutoAwayTarget(self.eta, self.temperature, self.virtualTarget);
            ...
            self.setTarget(self.virtualTarget, 'manual');
        }
