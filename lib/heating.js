                ...
                if (self.isAway === false) {
                    ...
                    if (self.eta > 0) {
                        ...
                        if (timeTillNextTarget - self.eta > 0) {
                            ...
                            timeTillNextTarget = timeTillNextTarget - self.eta + 65000;
                        } else {
                            aaTarget = self.calcAutoAwayTarget(self.eta, self.temperature, self.nextScheduledTarget);
                            timeTillNextTarget = timeTillNextTarget + 65000;
                        }

                        // Set current Target
                        self.setTarget(aaTarget, 'autoaway-override');
                        ...
                        self.nextTargetTimer = setTimeout(function() {
                            log.info({
                                home: self.homeId,
                                room: self.roomId
                            }, ' AutoAway Timer is executed in ' + Math.max(60001, timeTillNextTarget * 60 * 1000));
                            if (self.currentScheduledTarget !== self.nextScheduledTarget) {
                                var param = {
                                    'target': self.currentScheduledTarget
                                };
                                var eventData = helper.createRawEvent('autoaway-schedule-for-away', param);
                    ...
                    log.warn({
                        home: self.homeId,
                        room: self.roomId
                    }, ' Heating: AutoAway: There is now away state of the room so far --> We assume someone is at home');
...
Heating.prototype.createActivityForTargets = function(target, reason) {
    ...
    } else if (reason === 'autoaway-override') {
        if (target !== this.realTarget) {
            if (target < this.realTarget) {
                param = {
                    'target': target,
                    'oldtarget': this.realTarget
                };
                eventData = helper.createRawEvent('autoaway-leaving', param);
                this.fbRefActivity.push(eventData);
            } else if (target > this.realTarget) {
                param = {
                    'target': target,
                    'oldtarget': this.realTarget
                };
                eventData = helper.createRawEvent('autoaway-coming', param);
                this.fbRefActivity.push(eventData);
            }
        }
    }
