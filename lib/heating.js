...
function Heating(homeId, roomId) {
    ...
    this.fbRefUsesAutoAway.on('value', function(fbData) {
        log.info({
            home: this.homeId,
            room: this.roomId
        }, 'Heating: AutoAway change ' + fbData.val());
    ...
    this.fbRefIsAway.on('value', function(fbData) {
        log.info({
            home: this.homeId,
            room: this.roomId
        }, 'Heating: isAway changed tp ' + fbData.val());
            ...
            if (self.usesAutoAway) {
                ...
                if (!self.isAway) {
                    ...
                    log.info({
                        home: self.homeId,
                        room: self.roomId
                    }, ' Heating: AutoAway: timeTillNextTarget: ' + (timeTillNextTarget).toFixed(0) + ' minutes for target: ' + target);
                    ...
                    self.nextTargetTimer = setTimeout(function() {
                        ...
                        self.currentScheduledTarget = target;
                        self.setTarget(target, 'autoaway schedule timeout');
                        self.setVirtualTarget(target, 'autoaway schedule timeout');
                        self.planNextTarget();
                    ...
                    if (self.eta > 0) {
                        ...
                        if (timeTillNextTarget - self.eta > 0) {
                            ...
                            aaTarget = calcAutoAwayTarget(self.eta, self.temperature, self.nextScheduledTarget);
                            timeTillNextTarget = timeTillNextTarget + 1;
                        }

                        // Set current Target
                        self.setTarget(aaTarget, ' autoaway ');
                        // Schedule the next planNextTarget - Check
                        log.info({
                            home: self.homeId,
                            room: self.roomId
...
function calcAutoAwayTarget(eta, temperature, scheduledTarget) {
    ...
    if ((temperature <= minTemperature)|| (scheduledTarget <= minTemperature)) {
        log.info({
            home: this.homeId,
            room: this.roomId
        ...
        var heatupFactorForAutoAway = 30;
        var targetDiff = Math.min(maxTargetDiff, (Math.round(eta / heatupFactorForAutoAway * 2) / 2).toFixed(1));
        var newTarget = Math.max(minTemperature, scheduledTarget - targetDiff);
