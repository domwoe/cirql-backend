...
function MaxThermostat(homeId, roomId, thermostatId) {
    ...
    this.fbRefThermostat.once('value', function(fbThermostat) {
        ...
        if (fbThermostat.child('isPhysAddrSet').val()) {
            ...
            if (this.physAddr) {
                var self = this;
                this.nefit.setTrvAddress(this.physAddr, this.thermostatId, function(res, err) {
                    if (err) {
                        ...
                        }, 'MaxThermostat: Error while setting physAddr at Nefit Easy: ' + err);
                        ...
                        log.info({
                            ...
                            room: self.roomId,
                            thermostat: self.thermostatId
                        }, 'MaxThermostat: Response of setting physAddr: ' + res);

                        this.isPhysAddrSet = true;
                        this.fbRefThermostat.child('isPhysAddrSet').set(false);
