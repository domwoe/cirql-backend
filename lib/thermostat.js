  ...
  this.fbRefThermostat.child('fhem_desired-temp').on('value', function(fbThermostatSetpoint) {
  	...
  	if (this.fhemTargetTemperature === 12) {
  		...
  		// };
  		 ...
  		 log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'WARNING: fhem_measured-temp = 12. windowOpnMode');
...
Thermostat.prototype.testForConflict = function(target, fhemTarget) {
	if (target !== fhemTarget) {
  	...
  	// 		});
  		...
  		log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'WARNING: fhem_desired-temp !== thermostat target');
