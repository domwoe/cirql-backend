...
function Thermostat(homeId, roomId, thermostatId) {
  ...
  this.fbRefThermostat.child('target').on('value', function(fbThermostatSetpoint) {
  	...
  	this.testForConflict(this.targetTemperature,this.fhemTargetTemperature);
  ...
  this.fbRefThermostat.child('fhem_desired-temp').on('value', function(fbThermostatSetpoint) {
  	...
  	if (this.fhemTargetTemperature === 12) {
  		// var date = new Date();
  		// var problem = {
  		// 	home: this.homeId,
  		// 	room: this.roomId,
  		...
  		// };
  		 ...
  		 log.info({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'WARNING: fhem_measured-temp = 12. windowOpnMode');
	...
	this.fbRefThermostat.child('fhem_measured-temp').on('value', function(fbThermostatTemperature){
  	if(fbThermostatTemperature.val()){
		...
		theRoom.updateRoomTemperature();
		  ...
		  log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'Thermostat: fhem_measured-temp could not be found with url: ' + this.fbRefThermostat.child('commandAccepted').child('fhem_measured-temp').ref());
	}
   },
   this);
	...
	this.fbRefThermostat.child('manualChange').on('value', function(fbThermostatManualChange){
  	if(fbThermostatManualChange){
			...
			log.info({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'Thermostat: manualChange: ' + fbThermostatManualChange.val());
	}
	else {
		  log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'Thermostat: manualChange could not be found');
	}
   },this);
};
...
Thermostat.prototype.setTarget = function(value) {
	if(value) { 
		...
		value = (value < 5)  ? 5 : value;
			...
			log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'Thermostat: Invalid target temperature for thermostat');
	}
};
...
Thermostat.prototype.setExternalTemperature = function(obj) {
	...
	if(obj) { 
			...
			log.warn({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'Thermostat: Invalid external temperature object');
	}
};
...
Thermostat.prototype.testForConflict = function(target, fhemTarget) {
	if (target !== fhemTarget) {
  	...
  	// 		});
  		...
  		log.info({home: this.homeId, room: this.roomId, thermostat: this.thermostatId}, 'WARNING: fhem_desired-temp !== thermostat target');
