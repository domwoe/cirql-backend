...
this.fbRef.child('distance').on('value', function(fbData) {
  log.info({home: this.homeId, user: this.id}, 'User ' +this.name +' has distance: '+fbData.val());
  if(fbData.val() <= 500) {
    this.fbRef.child('isAway').set(false);
  }
  else {
     this.fbRef.child('isAway').set(true);
  }
},this);
