...
function Activity(homeId, roomId) {
    ...
    this.fbRefSystem.on('value', function(fbSystemStart) {
        if (fbSystemStart.val() === false) {
            ...
            this.lastEventDateUnix = moment(timestamp).unix();
            ...
            this.fbRefTemplate.once('value', function(fbTemplates) {
                ...
                this.fbRefRaw.on('child_added', function(fbRawEvent) {
                    if (fbRawEvent.val()) {
                        ...
                        var newRawEvent = fbRawEvent.val();
                        if (rawEvent['type']) {
                            ...
                            if (_.has(this.templates, eventType)) {
                                if (this.templates[eventType].show === true) {
                                    ...
                                    var eventDateUnix = moment(newEvent.date).unix();
                                    if (this.lastEventDateUnix !== null) {
                                        if (eventDateUnix > this.lastEventDateUnix - 1500) { // This delay can lead to some duplicates on system start, but it avoids that events at the same time are ignored. 
                                            ...
                                            if (this.lastRawEvent !== null) {
                                                if (this.lastRawEvent['type'] === newRawEvent['type'] && (eventDateUnix - this.lastEventDateUnix) <= 5 * 60000) {
                                                    if (this.lastDeEventRef !== null && this.lastEnEventRef !== null) {
                                                        this.removeEvent(newRawEvent);
                                                    }
                                                } else if (this.lastRawEvent['type'] === 'co2-red' && this.lastRawEvent['type'] === newRawEvent['type'] && (eventDateUnix - this.lastEventDateUnix) <= 8 * 60 * 60000) {
                                                    if (this.lastDeEventRef !== null && this.lastEnEventRef !== null) {
                                                        this.removeEvent(newRawEvent);
                                                    }
                                                } else if (this.lastRawEvent['type'] === 'resident-away' && newRawEvent['type'] === 'resident-home' && this.lastRawEvent['name'] === newRawEvent['name'] && (eventDateUnix - this.lastEventDateUnix) <= 1 * 60000) {
                                                    if (this.lastDeEventRef !== null && this.lastEnEventRef !== null) {
                                                        this.removeEvent(newRawEvent);
                                                        pushEvent = false;
                                                    }
                                                } else if (this.lastRawEvent['type'] === 'resident-home' && newRawEvent['type'] === 'resident-away' && this.lastRawEvent['name'] === newRawEvent['name'] && (eventDateUnix - this.lastEventDateUnix) <= 1 * 60000) {
                                                    if (this.lastDeEventRef !== null && this.lastEnEventRef !== null) {
                                                        this.removeEvent(newRawEvent);
                                                        pushEvent = false;
                                                    }
    ...
    // this.fbRefRaw.on('child_added', function(fbRawEvent) {
    //     if (fbRawEvent.val() !== null) {
    //         var key = fbRawEvent.key();
    //         var rawEvent = fbRawEvent.val();
    //         var rawEventDate = moment(rawEvent.date).unix();
    //         var currDate = moment().unix();
    //         if (currDate - rawEventDate >= 60 * 60 * 24 * 7) { // Older than 7 days
    //             if (key !== null) {
    //                 this.fbRefRaw.child(key).set(null);
    //             }
    //         }
    //     }
    // }, this);
    ...
    this.fbRefTemplate.child('cleanAllMsg').on('value', function(fbCleanAll) {
        if (fbCleanAll.val()) {
            if (fbCleanAll.val() === true) {
                ...
                log.info({
                    ...
                    room: this.roomId,
                    activity: this.roomId
                }, 'Msg Activity Logs deleted');
            }
        }
    }, this);

     this.fbRefTemplate.child('cleanAllRaw').on('value', function(fbCleanAll) {
        if (fbCleanAll.val()) {
            if (fbCleanAll.val() === true) {
                this.fbRefRaw.set(null);
                log.info({
                    home: this.homeId,
                    room: this.roomId,
                    activity: this.roomId
                }, 'Raw Activity Logs deleted');
            }
        }
    }, this);
...
Activity.prototype.removeEvent = function(newRawEvent) {
    log.info({
        home: this.homeId,
        room: this.roomId,
        activity: this.roomId
    }, 'Activity Logs: Consider to remove event ' + JSON.stringify(newRawEvent));

    if (newRawEvent['type'] === 'manual-target') {
        if (newRawEvent['name'] === this.lastRawEvent['name']) {
            this.lastDeEventRef.remove();
            this.lastEnEventRef.remove();
        }
    } else if (newRawEvent['type'] === 'change-schedule') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    } else if (newRawEvent['type'] === 'auto-away') {
        if (newRawEvent['name'] === this.lastRawEvent['name']) {
            this.lastDeEventRef.remove();
            this.lastEnEventRef.remove();
        }
    } else if (newRawEvent['type'] === 'change-mode') {
        if (newRawEvent['name'] === this.lastRawEvent['name']) {
            this.lastDeEventRef.remove();
            this.lastEnEventRef.remove();
        }
    } else if (newRawEvent['type'] === 'schedule-override') {
        if (newRawEvent['name'] === this.lastRawEvent['name']) {
            this.lastDeEventRef.remove();
            this.lastEnEventRef.remove();
        }
    } else if (newRawEvent['type'] === 'autoaway-override-currentScheduledTarget') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    } else if (newRawEvent['type'] === 'autoaway-override-nextScheduledTarget') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    } else if (newRawEvent['type'] === 'co2-red') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    } else if (newRawEvent['type'] === 'resident-away') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    } else if (newRawEvent['type'] === 'resident-home') {
        this.lastDeEventRef.remove();
        this.lastEnEventRef.remove();
    }
