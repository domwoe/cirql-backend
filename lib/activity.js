                                ...
                                if (this.templates[eventType].show === true) {
                                    ...
                                    if (this.lastEventDateUnix !== null) {
                                        if (eventDateUnix > this.lastEventDateUnix - 1500) { // This delay can lead to some duplicates on system start, but it avoids that events at the same time are ignored. 
                                            ...
                                            if (this.lastDeEventRef !== null) {
                                                if (this.lastDeEventRef.eventType === eventType &&  (eventDateUnix - this.lastEventDateUnix) <= 5*60000) {
                                                    if(eventType === 'manual-target') {
                                                        this.fbRefLogDe.remove(this.lastDeEventRef);
                                                        this.fbRefLogEn.remove(this.lastEnEventRef);
                                                    }
                                                }
                                            }
