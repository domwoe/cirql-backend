...
NetatmoAPI.prototype.getData = function(opt, cb) {
    ...
    function task(stationId, moduleId, type, which, cb) {
        var self = this;
        var params = {
            method: 'GET',
        ...
        request(params, function(error, response, body) {
            if (!error) {
                ...
                try {
                    ...
                    log.warn({
                        home: self.homeId
                    }, 'NetatmoAPI getMeasure: Could not parse body');

                    cb(e,null);

                }
                if ( jsonObj !== null && jsonObj.status === 'ok' && jsonObj.body) {

                    cb(null,jsonObj.body);
    ...
    if (typeof this.expires === 'undefined' || this.expires < Date.now() - 5000) {
        ...
        refreshAccessToken(self.refreshToken, function(err, res) {
            if (!err && res && res.hasOwnProperty('access_token')) {
                ...
                task.call(self, stationId, moduleId, type, which, cb);
            } else {
                console.log('Problem with getting new Netatmo access token');
        ...
        task.call(this, stationId, moduleId, type, which, cb);
    }
};
